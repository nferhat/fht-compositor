...
pub use actions::*;
use fht_compositor_config::{GestureAction, GestureDirection, GesturePattern, KeyPattern, WorkspaceSwitchAnimationDirection};
    ...
    pub fn process_input_event<B: InputBackend>(&mut self, event: InputEvent<B>) {
        ...
        match event {
            ...
            InputEvent::GestureSwipeUpdate { event } => {
                ...
                let swipe_state = active_monitor.swipe_state.as_ref().unwrap();
                let total_distance = (swipe_state.total_offset.x.powi(2) + swipe_state.total_offset.y.powi(2)).sqrt();
                
                if total_distance < swipe_state.direction_detection_threshold {
                    let active_monitor = self.fht.space.active_monitor_mut();
                    ...
                    let swipe_state = active_monitor.swipe_state.as_ref().unwrap();
                    let direction = match swipe_state.animation_direction {
