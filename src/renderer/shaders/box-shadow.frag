...
void main() {
    // the shader's element size will always fit the blur sigma
    vec4 rect = vec4(vec2(blur_sigma), size - vec2(blur_sigma * 2.0));
    ...
    frag_alpha *= rounded_box_shadow(rect.xy, rect.xy + rect.zw, v_coords * size, blur_sigma / 2.0, corner_radius);
