...
pub fn refresh(fht: &mut Fht) {
    crate::profile_function!();

    let manager_state = &mut fht.ext_workspace_manager_state;
    let mut changed = false;

    // Remove workspace groups of removed outputs.
    let outputs: HashSet<_> = fht.space.outputs().collect();
    ...
    for monitor in fht.space.monitors() {
